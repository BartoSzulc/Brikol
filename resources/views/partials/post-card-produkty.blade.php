<div class="col-span-1 flex">
    <div class="relative flex flex-col produkt__item bg-color7 transition-all duration-500 ease-in-out lg:p-30 p-5">
        <div class="pointer-events-none {{ $class ?? 'bg-white' }} absolute triangle-right -top-px -right-px w-[calc((115/340)*100%)] h-[120px] flex items-start justify-end z-0">
        </div>
        <div class="realization__item__image relatize z-10">
            <a class="overflow-hidden flex group" href="{{ get_permalink() }}">
                @if (has_post_thumbnail())
                <img class="h-[120px] w-full object-center object-contain transition-all duration-500 ease-in-out " src="{{ get_the_post_thumbnail_url() }}" alt="{{ get_the_title() }}">
                @endif
            </a>
